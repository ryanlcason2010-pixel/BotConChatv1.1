...
# Additional keywords for follow-up queries (treated as discovery/continuation)
FOLLOWUP_DISCOVERY_KEYWORDS = [
    'what else', 'anything else', 'other options', 'alternatives',
    'show me more', 'more frameworks', 'other frameworks',
    'what about', 'are there', 'do you have other', 'something else',
    'different one', 'different', 'nah', 'no', 'not that'
]
...
def detect_intent_with_context(
    ...
    """
    Detect intent with conversation context awareness.

    This improves detection for follow-up queries like "what else",
    "anything else", etc. by considering the previous conversation intent.

    Args:
        query: Current user query
        conversation_history: Previous messages in the conversation
        known_framework_names: List of known framework names
    ...
    is_followup = any(
        ...
        for phrase in FOLLOWUP_DISCOVERY_KEYWORDS
    )

    if is_followup and conversation_history:
        # Look for the last assistant response to understand context
        for msg in reversed(conversation_history):
            if msg['role'] == 'assistant':
                # Check if metadata has intent information
                metadata = msg.get('metadata', {})
                last_intent = metadata.get('intent')

                if last_intent:
                    # For follow-up queries, continue with discovery/diagnostic
                    # to show alternative frameworks
                    if last_intent in [INTENT_DIAGNOSTIC, INTENT_DISCOVERY]:
                        return INTENT_DISCOVERY, 0.75
                break

        # Even without metadata, if it's a follow-up, treat as discovery
        return INTENT_DISCOVERY, 0.6

    # Fall back to regular intent detection
    return detect_intent(query, known_framework_names, llm_client)
