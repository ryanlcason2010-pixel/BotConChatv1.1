...
def handle_discovery(
    ...
    # Extract framework data - DEDUPLICATE BY ID
    seen_ids = set()
    frameworks = []
    for result in search_results:
        fw_id = result.framework_data.get('id')
...
1. Framework name (in bold)
2. A 1-2 sentence description of what it does and when to use it
...
def _build_fallback_discovery_response(
    ...
    for fw_type, type_frameworks in types.items():
        ...
        for fw in type_frameworks[:5]:  # Show up to 5 per type
            name = fw.get('name', 'Unknown')
            difficulty = fw.get('difficulty_level', 'intermediate')
            use_case = fw.get('use_case', '')
            # Truncate use case for display
            if len(use_case) > 150:
                use_case = use_case[:150] + '...'
            lines.append(f"- **{name}** ({difficulty})")
            lines.append(f"  {use_case}")
        if len(type_frameworks) > 5:
            lines.append(f"  ...and {len(type_frameworks) - 5} more")
        lines.append("")
